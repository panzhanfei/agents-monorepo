...
| 应用 | 职责 |
|------|------|
| **`orchestrator`** | 编排：飞书/Webhook、任务状态、调用各 Agent、汇总可转发文案 |
| **`requirements-agent`** | **需求分析**：口头需求 → 结构化 PRD / 验收标准 / 风险 |
| **`coding-agent`** | 编码 Agent（当前多为登记与链路占位，演进中接真实改码） |
| **`review-agent`** | 审核：确定性门禁 + `.cursor/rules` 语义评审 |
| **`test-agent`** | 全量测试命令、聚合报告（飞书展示由 orchestrator 转发） |
| **`ops-agent`** | 运维：打包、产物校验、发布/远端操作与巡检 |

默认端口见 **`agents.config.yaml`**（本地占位约 **4010–4060**）。

## 仓库内其它路径

| 路径 | 说明 |
|------|------|
| `packages/pipeline-core` | 流水线步骤类型与各 Agent 间 DTO（避免各 app 魔法字符串） |
...
| `docs/ARCHITECTURE.md` | **架构**：契约、HTTP 栈、Skill 载荷等 |
| `docs/IMPLEMENTATION_ROADMAP.md` | **落地顺序**与自检清单 |

持久化 AI 编码约定见 **`.cursor/rules/*.mdc`**。

## 命令

详见 **`docs/FEISHU_COMMANDS.md`**（用户可见话术与运维规则）。
