...
| **Runner 心跳** | **本机 Runner → 云端 API** | **在线状态真源**：周期性上报（或 WebSocket 保活），载荷含 `runnerDeviceId`、`userId`、Runner/契约版本、可选已挂载 `projectId` 摘要；云端维护 **`lastSeenAt`**，用于 **仅向在线 Runner 派发可写盘任务**、桌面与各 **远端渠道（小程序、飞书 H5 等）** 展示「设备离线」及告警。**间隔、超时判定（如连续 N 周期未收到即离线）** 写入 `docs/ARCHITECTURE.md`。 |
...
| 资源 | 说明 |
|------|------|
| **本页下方「v1 设计清单与实施进度」** | **设计范畴对照表**（库表 / Redis / 队列 / 心跳 / 实时通道 / JWT / 业务与 Runner、前端契约）+ **实施进度表**（随做随记，只追加行） |
| `docs/ARCHITECTURE.md` | 建议维护：HTTP 契约、步骤机、Skill、**消息队列分区与消费语义**、**Runner 心跳字段与超时策略**、部署拓扑 |
| `docs/FEISHU_COMMANDS.md` | **（可选、后期）** 若接入 **飞书机器人** 时：指令词与内部 `action` 映射；**飞书 H5（应用内网页）** 与桌面/通用前端一样走 API，**不依赖** 此文 |
| `docs/WECHAT_MINIPROGRAM.md` | **（后续规划）** 微信小程序：接入方式、登录与 **产品内用户** 绑定、业务能力边界；与各渠道共用 **同一套 Express API / 任务模型** |
...
阶段一（Node + Agent）与阶段二（React）开战前，建议把下列项 **要么定稿进 `docs/ARCHITECTURE.md` / OpenAPI，要么在评审里显式记「暂缓 + 原因」**，避免实现到一半返工。
...
- **可观测性**：`traceId` / `requestId` **从网关到 Runner 日志**是否贯通；结构化日志字段表（见 README 后文「清晰日志」）。
...
|------|----------|----------------|------------------------|------|
| 2026-05-11 | 对齐 v1 实施范围：**库表、Redis、心跳、消息队列、实时通道（Socket/SSE）、JWT、业务与 Runner/前端接口**；README 增加进度记录方式 | 设计清单与「易漏项」写入 README，便于晚上/次日评审对照 | 逐项定稿进 `docs/ARCHITECTURE.md` 或 OpenAPI 时再逐行补充本表 | 本表专注 v1；只**追加**新行、**少改写**旧行，便于复盘 |
...
与上文 **「版本落地顺序」** 一致：**当前不设时间表** 做小程序与飞书；待 **Node + Agent** 与 **React（Electrobun）** 稳定后再做。原则不变：**客户仓只在本机 Runner 落盘**；各渠道只连 **Express**，共用 **账号、项目、任务、队列、心跳** 模型。用户不在电脑旁时，用手机 **查状态、审阅、确认/驳回、触发已编排步骤**（能力以实际实现为准）。
...
| **能做什么** | 查询任务状态、收摘要与告警、**确认/驳回**、触发「已编排好的」步骤等；小程序侧文档占位 `docs/WECHAT_MINIPROGRAM.md`；若将来启用机器人再维护 `docs/FEISHU_COMMANDS.md`（或与 `docs/CHANNEL_COMMANDS.md` 合并）。 |
