...
import type { IProjectsPageViewModel } from "./useProjectsPage";

export type IProjectsPageViewProps = { vm: IProjectsPageViewModel };

export const ProjectsPageView = ({ vm }: IProjectsPageViewProps) => {
  const {
    projectsQ,
    createM,
    deleteM,
          ...
          {filtered.length > 0 ? (
            <Flex align="center" justify="between" gap="3" wrap="wrap">
              <Text size="2" color="gray" highContrast={false}>
                共 {filtered.length} 条 · 每页 {pageSize} 条 · 第 {page} / {totalPages} 页
              </Text>
              <Flex gap="2">
                <Button
                  type="button"
